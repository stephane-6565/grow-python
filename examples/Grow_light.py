...
gpio.setup(16, GPIO.OUT) # LED control
...
def main():
    ...
    while True:
        ...
        if (6 <= now.hour <= 20) and (lux < light_value) :   # only turn on light between 06h00 and 20h00 while Lux value from light sensor is bellow the requested light value.
            print("Time: {a} | Light on (lux = {b})".format(a = now.hour , b = lux))     # Turn on light 
            GPIO.output(16, True) 
        else:    
            print("Time: {a} | Light off (lux = {b})".format(a= now.hour , b = lux))    # Turn off light 
